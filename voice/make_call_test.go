...
func TestValidatePostSuccess(t *testing.T) {
	p := Call{From: testNumber1, To: testNumber2, URL: "http://twimlbin.com/558a498f"}
	if nil != p.Validate() {
		t.Error("Validation of valid voice post failed.")
	}

	p = Call{From: testNumber1, To: testNumber2, ApplicationSid: "AP7e1e264cc0fd7143f3ef378e86bf3184"}
	if nil != p.Validate() {
		t.Error("Validation of valid voice post failed.")
	}

	p = Call{From: testNumber1, To: testNumber2, URL: "http://twimlbin.com/558a498f", SendDigits: "1234"}
	if nil != p.Validate() {
...
func TestValidatePostFailure(t *testing.T) {
	...
	p = Call{From: testNumber1}
	if nil == p.Validate() {
		t.Error("Validation of voice post missing From failed.")
	}

	p = Call{From: testNumber1, To: testNumber2}
	if nil == p.Validate() {
		t.Error("Validation of voice post missing URL & ApplicationSid failed.")
	}

	p = Call{From: testNumber1, To: testNumber2, URL: "http://twimlbin.com/558a498f", SendDigits: "1234a"}
	if nil == p.Validate() {
